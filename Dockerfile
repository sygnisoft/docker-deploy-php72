...
RUN apt-get update && buildDeps="libpq-dev libzip-dev gnupg acl git curl nano wget libpng-dev libgmp-dev ssh rsync" && apt-get install -y $buildDeps --no-install-recommends
...
RUN docker-php-ext-install pdo pdo_pgsql zip bcmath gd calendar gmp pcntl
