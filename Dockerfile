...
RUN apt-get update && buildDeps="libpq-dev libzip-dev gnupg acl git curl nano wget libpng-dev libgmp-dev ssh rsync libxml2-dev" && apt-get install -y $buildDeps --no-install-recommends
